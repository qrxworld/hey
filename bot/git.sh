...
if [ -z "$message" ]; then
    ...
    message=$(echo "$message" | bash "$HEY/hey.sh" $model_flag --prompt "Briefly summarize this git commit change. Output the message directly with no fluff, your response is echoed directly to the terminal so be brief. Multiple sentences ok but do not use newlines (its a commit message) " | tr -d '\n')
